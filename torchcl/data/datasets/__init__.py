...
def register_dataset(name, bypass_checks=False):
    """Registers a :class:`BaseDataset` subclass.

    This decorator allows to instantiate a subclass of BaseDataset 
    ...
    def register_dataset_cls(cls):
        if not bypass_checks:
            ...
            if not issubclass(cls, BaseDataset):
                raise ValueError(
                    f"Dataset ({name}: {cls.__name__}) must extend BaseDataset"
                )
...
def get_dataset(config, *args, **kwargs):
    """Builds a :class:`BaseDataset` from a config.
